...
class AnalysisResult {
  ...
  AnalysisResult({required this.message, required this.isSafe, this.error});
}
...
class WalkingBrain {
  ...
  Future<AnalysisResult> analyzeScene(Uint8List imageBytes, {bool detailedPrompt = false}) async {
    String prompt;
    if (detailedPrompt) {
      prompt = '【重要】あなたは視覚障害者の命を預かる歩行介助者です。' +
          '前方に見える景色、道の状況、障害物、建物、人、車両、信号機、標識など、' +
          'すべての重要な情報を具体的に日本語で説明してください。' +
          '少しでも危険の可能性があるものは必ず指摘してください。';
    } else {
      prompt = '【緊急重要】あなたは視覚障害者の歩行を支援する介助者AIです。この人の命と安全があなたの判断にかかっています。' +
