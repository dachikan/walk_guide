...
### AI介助者としての責任自覚
**すべてのAI（Gemini・Claude・ChatGPT）は視覚障害者用介助者として機能する**
...
### 現在のバージョン
**v0.0.1+1** (2026年2月16日現在)

### バージョン管理体系
- **0.0.1+x**: 内部改良版（+1, +2, +3...）
- **0.0.2+1**: 協力者配布版（APK配布時にメジャーバージョンアップ）
- **0.0.x+1**: 機能追加・修正時の小数点バージョンアップ
...
#### AI状態確認コマンド
- 「AI」「エーアイ」「現在のAI」「どのAI」
  → 現在使用中のAI（Gemini/Claude/ChatGPT）を音声で通知
...
#### 1. AppState.normal（通常状態）
- **マイクボタン色**: 🔵 **青色** (`Colors.blue`)
- **アイコン**: `Icons.mic_none` (マイクオフ)  
- **表示文字**: 「🔄 通常解析中」
- **動作**: 5秒間隔の自動前方解析実行中

#### 2. AppState.listening（音声コマンド待機中）
- **マイクボタン色**: 🔴 **赤色** (`Colors.red`)
- **アイコン**: `Icons.mic` (マイクオン)
- **表示文字**: 「🎤 音声待機中」  
- **動作**: 音声コマンドを確実に受け付け中

#### 3. AppState.processing（AI分析・音声出力中）
- **マイクボタン色**: 🔴 **赤色** (`Colors.red`)
- **アイコン**: `Icons.mic` (マイクオン保持)
- **表示文字**: 「⚙️ 処理中」
- **動作**: AI分析またはTTS音声出力を実行中

### ⚠️ 重要: 状態と色の完全一致保証
- **listening状態になった瞬間に赤色に変更**
- **processing状態でも赤色維持**  
- **normal状態に戻った瞬間に青色に復帰**
- **色と状態が一致しない場合は実装エラー**
